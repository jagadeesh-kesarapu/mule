...
import org.mule.runtime.module.extension.internal.config.ParameterParsingDescriptor;
 ...
 * @since 4.0
 */
public final class GuachiParserDelegate
{
    ...
    private ParameterParsingDescriptor getParameterParsingDescriptor(ComponentBuildingDefinition.Builder definition, final String name, MetadataType type, Object defaultValue)
    {
        ...
        return new ParameterParsingDescriptor(definition, name, hyphenize(name), type, defaultValue);
    }
    ...
    ValueResolver parseElement(ParameterParsingDescriptor parameterDescriptor)
    {
        ...
        final ComponentBuildingDefinition.Builder definition = parameterDescriptor.getDefinition();
        ...
        parameterDescriptor.getMetadataType().accept(new MetadataTypeVisitor()
        {
            ...
            {
                resolverReference.set(getResolverFromValue(getAttributeValue(definition, fieldName, defaultValue), metadataType));
            }
            ...
            {
                resolverReference.set(parseCollection(definition, fieldName, elementName, singularize(elementName), defaultValue, arrayType));
            }
            ...
            public void visitDictionary(DictionaryType dictionaryType)
            {
                ...
                resolverReference.set(parseMap(definition, fieldName, parentName, childname, defaultValue, dictionaryType));
            }

            @Override
            public void visitObject(ObjectType objectType)
            {
                resolverReference.set(parsePojo(definition, fieldName, elementName, objectType, defaultValue));
            }

            @Override
            public void visitDateTime(DateTimeType dateTimeType)
            {
                resolverReference.set(parseCalendar(definition, fieldName, dateTimeType, defaultValue));
            }

            @Override
            public void visitDate(DateType dateType)
            {
                resolverReference.set(parseDate(definition, fieldName, dateType, defaultValue));
            }
        });
     ...
     */
    public ResolverSet getResolverSet(ComponentBuildingDefinition.Builder definition, List<ParameterModel> parameterModels) throws ConfigurationException
    {
        return getResolverSet(definition, parameterModels, ImmutableMap.of());
    }
     ...
     * @throws ConfigurationException in case of invalid configuration
     */
    public ResolverSet getResolverSet(ComponentBuildingDefinition.Builder definition, List<ParameterModel> parameterModels, Map<String, List<MessageProcessor>> nestedOperations) throws ConfigurationException
    {
        ...
        for (ParameterModel parameterModel : parameterModels)
        {
            ...
            {
                ValueResolver<?> resolver = getValueResolver(definition, parameterModel);
    ...
    private void getValueResolver(ComponentBuildingDefinition.Builder definition, ParameterModel parameterModel)
    {
        if (parameterModel.getExpressionSupport() == ExpressionSupport.LITERAL)
        {
            AttributeDefinition.Builder builder = fromSimpleParameter(parameterModel.getName(), value -> new ParsedParameter(parameterModel, new StaticValueResolver(value)));
            Object defaultValue = parameterModel.getDefaultValue();
            if (defaultValue != null) {
                builder.withDefaultValue(defaultValue);
            }

            definition.withSetterParameterDefinition(parameterModel.getName(), builder.build());
        } else {
            parseElement(getParameterParsingDescriptor(definition, parameterModel.getName(), parameterModel.getType(), parameterModel.getDefaultValue()));
